...
| #221 | **CODEX audit 5th (security) + 6th (live pentest) — re-verify แบบ live ไม่ใช่แค่อ่าน static, ปิด 4 phase ในรอบเดียว (2026-07-22)** — สอง audit doc แยกกัน (`CODEX_Audit_5th_part_security.md` 12 finding SEC5-HIGH/MED/LOW, `CODEX_Audit_6th_live_pentest_summary.md` 9 finding LIVE-HIGH/MED/LOW/PASS จาก live pentest ที่ CODEX รันจริงกับ production) — ตรวจสอบซ้ำทุกข้อกับโค้ด+ระบบจริง (SSH เข้า hdy-edge/vss-edge, EMQX API สด, log จริง) ก่อนตัดสินใจ ไม่เชื่อ static claim เฉยๆ. **Phase 0-1** (hygiene, เสี่ยงศูนย์): edge `cameras-config.json` chmod 600 self-heal, `edge/env.template` แก้คำสั่งผิด (`CAMERA_SECRET_KEY` ต้อง copy จาก central ไม่ใช่ generate ใหม่), `.DS_Store` cleanup, `lpr-receiver` disable x-powered-by. **Phase 2** (receiver DoS hardening): token pre-check **ก่อน** `express.raw()` ใน `routes/lpr.js`/`routes/face-push.js` — ระหว่างตัดสินใจ response code พบว่า "burst" ที่ audit เห็นจริงๆ คือ **flood ที่กำลังยิงอยู่ต่อเนื่อง** (~2 req/s unknown-token `/face-push` ตั้งแต่ 2026-07-17) ไม่ใช่แค่ทฤษฎี — verify สด 93 hit ถูกดักได้ในนาทีแรกหลัง deploy; multer 2.1.1→2.2.0 (DoS advisory fix). **Phase 3**: `INTERNAL_API_SECRET` fallback เปลี่ยนเป็น fail-fast เมื่อ `NODE_ENV=production` เท่านั้น (dev ยังคง ephemeral fallback ได้) — ต้องเพิ่ม `NODE_ENV` เข้าระบบเป็นครั้งแรก (ไม่เคยมีมาก่อนเลย). **Phase 4**: ปิด legacy `POST /lpr` เป็น 410 (หลักฐาน log 0 hit ตลอดประวัติทุกที่), เพิ่ม EMQX ACL rule ให้ 8 user ที่ไม่มี rule มาก่อน (central, prep สำหรับ deny-by-default ในอนาคต ไม่ได้ flip จริง), narrow `lpr-receiver` bind กลาง `0.0.0.0`→`127.0.0.1` (ยืนยัน Cloudflare tunnel route ไป `localhost:3003` จริงจาก dashboard ก่อนแคบ), rotate cloudflared tunnel token (หลุดเข้า session transcript ระหว่างตรวจ `ps aux`). **บทเรียนกระบวนการ:** พลาด commit โดยไม่รอ confirm แยก 1 ครั้ง (WA#4 STOP violation, ยอมรับตรงๆ กับเจ้าของ); พลาด deploy edge สำหรับ Phase 4c ไป 1 รอบ (GOTCHAS #116); เปิดไฟล์ `src/.env` เต็มไฟล์ทำให้ secret ทั้งหมดหลุดเข้า transcript ครั้งที่ 2 (ประเมิน severity แล้วต่ำกว่า cloudflared token เพราะเป็น localhost-bound services). Commits: `6525a02` `7fd4204` `2fc9a99` `82c5963` `088fedf` `38aabb6` `d52142a` | `CODEX/CODEX_Audit_5th_part_security.md` (§7) · `CODEX/CODEX_Audit_6th_live_pentest_summary.md` (§9) · GOTCHAS #115–#116 · ROADMAP |
| #222 | **Gate `detect-model`/`delete-media` ด้วย shared secret (`CAMERA_SECRET_KEY` เดิม) แทน NanoMQ auth เต็มรูปแบบ — scope ให้แคบตาม threat model จริง ไม่ทำ "ให้ครบ" เกินจำเป็น** — SEC5-HIGH-003 (NanoMQ edge `allow_anonymous=true`) เดิมดูเหมือนต้องแก้เต็มรูปแบบเพราะตอนนี้แบก command channel ทำลายข้อมูลได้ (`delete-media`) — แต่ตรวจ `docs/LOGIC_edge-ingester-divergence.md` แล้วพบว่ากล้อง Bosch ที่ site hdy/vss publish เข้า **local NanoMQ ตรงๆ** (ไม่ผ่าน node process) → เปิด auth เต็มรูปแบบ = ต้องตั้งค่าในตัวกล้องทุกตัวจริง (firmware-level, เสี่ยงกล้องหลุดทั้งไซต์). เลือกทางเลือกที่ถูกกว่าแทน: `edge-config-agent.js` เช็ค `secret` field ใน payload เทียบกับ `CAMERA_SECRET_KEY` (timing-safe compare) ก่อนรัน `detectModel()`/`deleteCameraMedia()` — reuse key ที่มีอยู่แล้ว (shared ทุกเครื่องอยู่แล้วสำหรับ credential encryption) แทน generate secret ใหม่ เพราะเป็นแค่ equality check ไม่ใช่ cryptographic operation ใหม่ที่จะ mix purpose จริงจัง. Secret ผิด/ไม่มี = log แล้ว**เงียบ ไม่ตอบ `_result` ใดๆ** (กัน oracle ให้คนสุ่ม token ไม่รู้ว่าผิดตรงไหน). ปิดเฉพาะ 2 channel ที่สร้างเองเซสชันนี้ **ไม่ครอบ** `scan-nvr` เดิม (SEC5-MED-004, ยังไม่ทำ) และ **ไม่แก้** NanoMQ anonymous โดยรวม | `src/edge-config-agent.js` (`hasValidCommandSecret`) · `src/routes/cameras.js` |
| #223 | **เลื่อน EMQX `no_match=deny` ไปผูกกับ multi-site rollout, เลื่อน `CAMERA_SECRET_KEY` rotation แบบไม่มีกำหนด — ทั้งคู่มี "เหตุผลที่ควรทำ" แต่ไม่ใช่ "ต้องทำตอนนี้"** — **(a) `no_match=deny`:** central EMQX auth อยู่แล้ว (live pentest 6th ยืนยัน) — flip นี้แค่จำกัด client ที่ auth ผ่านแล้วให้อยู่ใน topic lane ตัวเอง (defense-in-depth ชั้นเสริม ไม่ใช่ปิดรูรั่ว authentication) เสี่ยง fleet-breaking ถ้า site/topic ไหนไม่ได้ inventory ครบ (audit เดิมเองก็จัดไว้ใต้ "Before Wider Multi-Site Rollout" ไม่ใช่ Immediate) — prep เสร็จแล้ว (10 user มี ACL rule ครบ + ลบ orphan `edge-zztest*`/`cam-b3100i-2` แล้ว) รอ flip จริงตอน multi-site rollout ที่จะต้อง provision site ใหม่อยู่แล้วเป็นงานปกติ. **(b) `CAMERA_SECRET_KEY` rotation:** หลุดเข้า session transcript ตอนแก้ `LPR_BIND_HOST` (ต้อง Read ทั้งไฟล์ `.env`) — ประเมิน threat model แล้วไม่ร้ายแรงเท่า cloudflared token เพราะต้องมีทั้ง key + ciphertext file พร้อมกันถึงมีประโยชน์ (คนที่เข้าถึงไฟล์ได้ระดับนั้นอ่าน `.env` เอาค่า key ตรงๆ ได้อยู่แล้ว) — เขียน migration script (`scripts/rotate-camera-secret-key.js`, self-tested ด้วย throwaway key คู่, decrypt-old→encrypt-new พร้อม backup+idempotent+wrong-key-detection) ไว้พร้อมใช้ แต่ **generate key จริง + แก้ `.env` 3 เครื่องต้องเป็นเจ้าของทำเอง** — Claude ไม่สามารถ generate/handle ค่า key จริงได้โดยไม่ทำให้หลุดเข้า transcript ซ้ำอีกรอบ (กฎเดียวกับที่ปฏิเสธ paste cloudflared token ให้) | `scripts/rotate-camera-secret-key.js` · ROADMAP ("CODEX Audit 5th (security)...") |

---

<sub>End of DECISIONS.md (index) · Companion to CLAUDE.md · Updated 2026-07-22 (decisions #152–#223)</sub>
