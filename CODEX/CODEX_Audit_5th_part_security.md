...
| HIGH-003 | **Partially mitigated (2026-07-22), NanoMQ itself still open.** Full NanoMQ auth turned out to be a camera-firmware-touching change (Bosch cameras publish ONVIF events straight to edge NanoMQ over LAN, per `docs/LOGIC_edge-ingester-divergence.md` — not through a node process), so deferred rather than rushed. Instead gated the two channels this created the actual destructive-command risk: `edge-config-agent.js` now requires a `secret` field (compared against `CAMERA_SECRET_KEY`, timing-safe) on `_config/detect-model` and `_config/delete-media` before acting — wrong/missing secret is logged and silently dropped, no reply. Verified live on hdy-edge: wrong-secret publish rejected, correct-secret publish processed normally, both against a synthetic non-existent camera_id (no real data touched). NanoMQ anonymous bind/`0.0.0.0` itself is unchanged — this only closes the specific destructive-command path, not the broader ingest-spoofing surface. |
